...
int verificaParam(float vetor_RSSI[],float vetor_SNR[],int contador_perda){
  ...
  int RSSImi = encontraMinimo(vetor_RSSI,8);
  
  int RSSImax = encontraMaximo(vetor_RSSI,8);

  int SNRmi = encontraMinimo(vetor_SNR,8);
  int SNRmax = encontraMaximo(vetor_SNR,8);
  
  float RSSInorm = (mediaRSSI - (-137.5))/ ((-104.0) - (-137.5));
  float SNRnorm = (mediaSNR - 0.75)/ (33.5 - (0.75));
  
  
  Serial.println("RSSI1: "+String(RSSInorm));
  Serial.println("SNR1: "+String(SNRnorm));
  
  float porcCt = (contador_perda*100)/8;
  porcCt = porcCt/100;
  Serial.println("PORC : "+String(porcCt));
  /*
  if(fabs(SNRmax - SNRmi) < 1e6 && porcCt > 0){
    SNRnorm = 1;
      
  }else if(fabs(SNRmax - SNRmi) < 1e6 && porcCt == 0){
    SNRnorm = 0;
  }
  if(fabs(RSSImax - RSSImi) < 1e6  && porcCt > 0){
    RSSInorm = 1;
  ...
  */
  ...
  Serial.println("RSSI2: "+String(RSSInorm));
  Serial.println("SNR2: "+String(SNRnorm));
  ...
  int meio = j;
  while(inicio<fim){
    double intervalo_ideal = R - vetor_Parametros[meio].R_v;
    if(intervalo_ideal < 0.05 && intervalo_ideal > -0.05){
      //Serial.println("0");
      break;
    }
...
void loop() {
  ...
  String str_read;
  int contador = 0;
  unsigned long period = 1000*50;
  
  float vetor_RSSI[8] = {0,0,0,0,0,0,0,0};
  float vetor_SNR[8] = {0,0,0,0,0,0,0,0};
